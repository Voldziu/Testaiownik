...
def render_questions_manager():
    ...
    if st.session_state["user_questions"]:
        ...
        st.info(f"💡 Dodano **{len(st.session_state['user_questions'])}** własnych pytań")
    else:
...
def start_test(quiz_id: str, total_questions: int, user_questions: List[str]):
    ...
    try:
        with st.spinner("Rozpoczynanie testu..."):
            ...
            # Pobieranie potwierdzonych tematów z session_state
            confirmed_topics = st.session_state.get("confirmed_topics", [])
            
            if not confirmed_topics:
                st.error("❌ Brak potwierdzonych tematów. Wróć do poprzedniego kroku i potwierdź tematy.")
                return
            ...
            request_data = {
                ...
                "confirmed_topics": confirmed_topics  # Dodanie wymaganych tematów
            }
            
            # Wywołanie endpointu /quiz/{quiz_id}/start
            response = api_client.start_quiz(
                quiz_id=quiz_id,
                **request_data
            )
