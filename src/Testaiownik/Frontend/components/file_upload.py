...
def _render_indexing_setup():
    ...
    with col1:
        if st.button("🚀 Rozpocznij indeksowanie", type="primary", use_container_width=True):
...
def _render_indexing_monitor():
    ...
    with col2:
        if indexing_complete:
            if st.button("➡️ Przejdź do tematów", type="primary", use_container_width=True):
                st.rerun()
        else:
            st.button("⏳ Indeksowanie w toku...", disabled=True, use_container_width=True)
