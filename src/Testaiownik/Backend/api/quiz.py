...
async def change_questions_number(
    ...
    try:
        ...
        if quiz_session.status == "completed":
            raise HTTPException(
                status_code=400, detail="Cannot change question count of completed quiz"
            )

        old_count = quiz_session.total_questions

        # Update question count in database
        from ..database.crud import update_quiz_session

        update_quiz_session(
            quiz_session_id, total_questions=questions_data.total_questions
        )
        ...
        # Check if additional generation is needed
        questions_data_stored = quiz_session.questions_data or {}
        current_questions = len(
            questions_data_stored.get("all_generated_questions", [])
        )
        additional_generation_required = (
            questions_data.total_questions > current_questions
        )
        ...
        return {
            ...
            "old_count": old_count,
            "new_count": questions_data.total_questions,
            "additional_generation_required": additional_generation_required,
        }
